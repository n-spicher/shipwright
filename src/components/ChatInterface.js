...
const ChatInterface = ({ documentUrl, documentName }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // In the MVP, we'll just simulate a response since the backend is not built yet
      // This is a placeholder for the actual API call to the backend
      
      // Simulate API delay
      await new Promise(resolve => setTimeout(resolve, 1500));
      
      // Include contractor type in the response
      const contractorInfo = `As a ${contractorType}, `;
      
      // Mock response with page citation
      let responseText;
      if (input.toLowerCase().includes('cost') || input.toLowerCase().includes('price')) {
        responseText = `${contractorInfo}based on the document, the estimated costs for materials range from $15,000 to $25,000 depending on quality grade. Labor costs are approximately $30-45 per hour. [Page 42]`;
      } else if (input.toLowerCase().includes('timeline') || input.toLowerCase().includes('schedule')) {
        responseText = `${contractorInfo}the project timeline is estimated at 4-6 months for completion, with foundation work taking 3-4 weeks. [Page 17]`;
      } else if (input.toLowerCase().includes('material') || input.toLowerCase().includes('supplies')) {
        responseText = `${contractorInfo}the document specifies using Type II Portland cement for the foundation and suggests grade-A lumber for framing. Electrical requirements are detailed on pages 78-82. [Page 56]`;
      } else {
        responseText = `${contractorInfo}I've found some information that might help with your query. For more specific details, could you clarify what aspect of the construction project you're interested in? The document contains details on budgeting (pages 40-55), scheduling (pages 15-30), and material specifications (pages 50-90).`;
      }
