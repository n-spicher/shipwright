...
from app.utils.firebase_auth import initialize_firebase, get_current_user_from_token
...
async def request_logging_middleware(request: Request, call_next):
    ...
    logger.info(f"[request] start request_id={request_id} method={request.method} path={request.url.path}")
    try:
        response = await call_next(request)
    except Exception as e:
        logger.error(f"[request] error request_id={request_id} method={request.method} path={request.url.path} err={str(e)}")
        raise
    logger.info(f"[request] end request_id={request_id} status={response.status_code} path={request.url.path}")
    return response
...
def get_current_user(authorization: str = Header(None), db: Session = Depends(get_db)) -> models.User:
    ...
    if not authorization:
        logger.error("[get_current_user] No Authorization header provided")
        raise HTTPException(status_code=401, detail="Authorization header required")
    
    # Extract token from "Bearer <token>" format
    parts = authorization.split()
    if len(parts) != 2 or parts[0].lower() != "bearer":
        logger.error(f"[get_current_user] Invalid Authorization header format: {authorization[:20]}...")
        raise HTTPException(status_code=401, detail="Invalid Authorization header format. Expected: Bearer <token>")
    
    token = parts[1]
    logger.info(f"[get_current_user] Bearer token received (length={len(token)})")
    
    # Verify token and get/create user
    try:
        current_user = get_current_user_from_token(token, db)
        logger.info(f"[get_current_user] Authenticated user_id: {current_user.id}")
        return current_user
...
@app.post("/ask")
async def ask(request: ChatRequest, current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
        logger.info(f"[ask] Processing request for user_id: {current_user.id}, message: {request.message[:50]}...")
            
        # Get user keywords from database
        user_keywords = db.query(Keyword).filter(Keyword.user_id == current_user.id).all()
...
async def upload_pdf(
    ...
    try:
        logger.info(f"[upload-pdf] Processing PDF for user_id: {current_user.id}, filename: {file.filename}")
        ...
        async def generate():
            try:
                async for progress in process_pdf(file_content, file.filename, current_user.id):
